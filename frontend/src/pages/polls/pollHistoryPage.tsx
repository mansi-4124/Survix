...
const PollHistoryPage = () => {
  ...
  return (
    <PageReveal asChild>
      ...
      {isLoading ? <PageStateCard description="Loading poll history..." /> : null}
      {isError ? <PageStateCard tone="error" description="Could not load polls." /> : null}

      {!isLoading && !isError && (polls?.length ?? 0) === 0 ? (
